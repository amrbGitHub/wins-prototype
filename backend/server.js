...
// POST /api/reflections/chat — stateless weekly review Q&A turn (no auth — stateless like planner/chat)
app.post('/api/reflections/chat', async (req, res) => {
  try {
    ...
    const goalsList = goals.map((g, i) =>
      `${i + 1}. "${g.title}"${g.description ? ` — ${g.description}` : ''}${g.successCriteria ? ` (Success criteria: ${g.successCriteria})` : ''}`
    ).join('\n')
...
- "evaluation": 2–4 paragraphs — an honest, encouraging written assessment of their overall progress, specific to what they shared
- "suggestions": 3–5 short actionable ideas to help them make better progress (each a single sentence)

Until you have enough info, always respond with:
...
`.trim()
    ...
    try {
      ...
      const fallbackMsg = (content?.trim() || 'Sorry, I had trouble responding. Please try again.')
        .replace(/\{[\s\S]*\}\s*$/, '').trim()
      parsed = { message: fallbackMsg, done: false, evaluation: '', suggestions: [] }
    }

    const cleanMessage = (parsed.message || '').replace(/\{[\s\S]*\}\s*$/, '').trim()
...
app.get('/api/reflections', verifyToken, async (req, res) => {
  try {
    let query = supabase
      .from('reflections')
      .select('*')
...
app.post('/api/reflections', verifyToken, async (req, res) => {
  try {
